...
fn next_generation_fails_constraints_after_retries() {
    ...
    let mut member = PopMember::from_expr_with_birth(MemberId(0), None, Some(0), leaf_expr(), dataset.n_features);
...
fn reg_evol_cycle_skips_replacement_when_configured() {
    ...
    let full_dataset = TaggedDataset::new(&dataset, baseline_loss);
    let member = PopMember::from_expr_with_birth(MemberId(0), None, Some(0), leaf_expr(), dataset.n_features);
