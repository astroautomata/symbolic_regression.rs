...
fn tournament_penalizes_frequent_sizes_when_enabled() {
    ...
    let mut a = PopMember::from_expr_with_birth(MemberId(1), None, Some(0), leaf_expr(), 1);
    let mut b = PopMember::from_expr_with_birth(MemberId(2), None, Some(0), leaf_expr(), 1);
    a.complexity = 1;
    b.complexity = 3;
    a.cost = T::one();
    b.cost = T::one();
